			...
			case WebSocketServer::Text:
				if (!ClientActive)
				{
					if (!memcmp(data, "ImInit", 6))
					{
                        ...
                        ci::gl::Texture2dRef fontTex = ci::gl::Texture2d::create( GL_TEXTURE_2D, (GLuint) fontAtlas->TexID, fontAtlas->TexWidth, fontAtlas->TexHeight, true );
                        auto channel = ci::Surface::create( fontTex->createSource() )->getChannelAlpha();
                        auto flippedChannel = ci::Channel8u::create( channel.getWidth(), channel.getHeight() );
                        ci::ip::flipVertical( channel, flippedChannel.get() );
                        PreparePacketTexFont( flippedChannel->getData(), flippedChannel->getWidth(), flippedChannel->getHeight() );
                        SendPacket();
					}
				}
				...
				else if (strstr((char *)data, "ImClipboard="))
				{
					...
					ImGui::GetIO().SetClipboardTextFn( ImGui::GetIO().ClipboardUserData, clipboard );
				}
				break;
			// Binary message
